...
function gen_setters(): void {
	$args = '0';

	for($i = 2; $i <= N; ++$i) {
		$args .= ', ' . $i - 1;
		echo "\tpad.SetFunc$i($i, add$i, $args)" . PHP_EOL;
	}
}

# function definitions generator
function gen_defs(): void {
	$args = $calc = 'a';
	...
	for($i = 2; $i <= N; ++$i) {
		...
		echo "func add$i($args int) int	{ return $calc }" . PHP_EOL;
	}
}

# result calculator
function calc_result(): void {
	$arr = [ 1, 2, 3 ];
